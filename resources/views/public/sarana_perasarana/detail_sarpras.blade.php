    ...
    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
      <div class="grid md:grid-cols-2">
        ...
        <div class="p-8">
          ...
          <!-- Judul + Tombol Peminjaman -->
          <div class="flex justify-between mb-3">
            <h2 class="text-3xl font-bold text-gray-800">
              {{ $sarpras->nama_ruangan ?? $sarpras->nama_proyektor ?? 'Nama Sarpras Tidak Diketahui' }}
            </h2>

            <!-- Tombol Peminjaman (Diperbaiki) -->
            <div>
              @if(!$mainPeminjaman || $mainPeminjaman->status_peminjaman === 'Tersedia' || $resourceStatus === 'Tersedia')
                <a href="{{ route('public.peminjaman.create', [
                    'sarpras_type' => $type,
                    'sarpras_id' => $sarpras->id_ruangan ?? $sarpras->id_proyektor
                  ]) }}"
                  class="text-sm px-3 py-1.5 bg-[#179ACE] text-white rounded hover:bg-[#137aa3]">
                  Ajukan Peminjaman
                </a>
              @else
                <span class="text-sm text-red-600">Sarpras sedang dipakai</span>
              @endif
            </div>
          </div>
          ...
          <div class="border-t pt-5">
            ...
            @if(Auth::check())
              <a href="{{ route('public.feedback.index', [
                    'id_sarpras' => $sarpras->id_ruangan ?? $sarpras->id_proyektor,
                    'type'       => $type
                ]) }}"
                class="text-sm px-3 py-1.5 bg-[#656564] text-white rounded hover:bg-[#404040]">
                Berikan Feedback
              </a>
            @else
              <p class="text-sm text-gray-600 mb-3">Silakan login untuk dapat memberikan feedback.</p>
              <a href="{{ route('login') }}"
                 class="inline-flex items-center justify-center bg-gray-600 text-sm hover:bg-gray-700 text-white font-normal px-4 py-1.5 rounded transition">
