...
<body class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-100">
  ...
  <div class="w-full max-w-5xl bg-white/90 backdrop-blur-lg rounded-2xl border border-gray-200 grid grid-cols-1 md:grid-cols-2 overflow-hidden animate-fadeIn">
    ...
    <div class="p-8 md:p-10">
      ...
      <p class="text-gray-500 text-sm text-center mb-6">Isi formulir di bawah untuk mendaftar</p>
      ...
      <form method="POST" action="{{ route('register') }}" class="space-y-5">
        ...
        <div>
          ...
          <input id="nama" name="nama" type="text" placeholder="Nama Lengkap" value="{{ old('nama') }}" required
        ...
        <div>
          ...
          <input id="password_confirmation" name="password_confirmation" placeholder="Konfirmasi Password" type="password" required
            class="w-full border rounded-lg py-2.5 text-sm px-3 shadow-sm focus:bg-blue-50  transition">
        </div>
        ...
        <p class="text-center text-sm text-gray-600 mt-5">
          Sudah punya akun?
          <a href="{{ route('login') }}" class="text-blue-600 font-semibold hover:text-blue-800 transition">Masuk di sini</a>
        </p>
      </form>
    </div>
  </div>
